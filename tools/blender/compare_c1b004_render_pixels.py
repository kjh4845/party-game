...
def main():
    if len(sys.argv) != 3:
        raise SystemExit("usage: compare_c1b004_render_pixels.py EXPECTED_DIR ACTUAL_DIR")
    ...
    for name in sorted(set(expected_names) | set(actual_names)):
        ...
        entry = {
            "name": name,
            "expectedExists": os.path.isfile(expected_path),
            "actualExists": os.path.isfile(actual_path),
        }
            ...
            entry.update(
                {
                    ...
                    "exactPixelMatch": expected_hash == actual_hash
                    and expected_dimensions == actual_dimensions,
    ...
    print(
        "C1B004_RENDER_REPRODUCTION_JSON="
